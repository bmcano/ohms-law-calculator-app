...
fun NavGraphBuilder.calculatorResistance(
    navHostController: NavHostController,
) {
    composable(
        route = Screen.CalculatorResistance.route,
        ...
        val viewModel: CalculatorViewModel = viewModel(factory = CalculatorViewModelFactory(Formulas.Resistance.EQ1))
        val ohmsLaw by viewModel.ohmsLaw.collectAsState()
        val formulaDetails by viewModel.formulaDetails.collectAsState()

        CalculatorScreen(
            title = stringResource(R.string.home_resistance),
            openMenu = openMenu,
            reset = reset,
            ohmsLaw = ohmsLaw,
            selectedFormulas = DropdownLists.FORMULAS_RESISTANCE,
